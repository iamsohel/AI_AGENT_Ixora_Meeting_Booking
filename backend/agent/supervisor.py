...
def classify_intent_node(state: dict, llm) -> dict:
    ...
    if cached_intent:
        ...
        logger.info(
            f"✅ Using cached intent classification for: {last_user_message[:50]}"
        )
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are an intent classifier for Ixora Solution's AI assistant.
            ...
            - Any negative word during booking in progress
            ...
            - If user cancels the booking during middle of a booking in progress → RAG
            - If user says something negative during booking in progress (no, cancel, stop, nevermind, not interested, let's talk about, tell me about) → RAG
            - If user wants to know about company/services instead of booking → RAG
    ...
    response = chain.invoke(
        {"chat_history": chat_history, "message": last_user_message}
    )
...
def route_to_agent(state: dict) -> Literal["rag", "booking"]:
    ...
    """
    # Trust the LLM's classification completely - no hardcoded overrides
...
def handle_agent_switch(state: dict, llm) -> dict:
    ...
    if previous_mode == "rag" and current_mode == "booking":
        ...
        if (
            not any(
                keyword in last_ai_message.lower() for keyword in transition_keywords
            )
            if last_ai_message
            else True
        ):
            state["messages"].append(
                AIMessage(
                    content="Great! I'll help you book a meeting. What date works best for you?"
                )
            )
