...
def add_pdf_to_vectorstore_with_retry(pdf_path: str, batch_size: int = 5, delay: int = 2):
    """
    Add a new PDF document to the existing vector store with rate limiting.
    ...
    """
    print("=" * 60)
    print("Adding New Document to Vector Store (with Rate Limiting)")
    print("=" * 60)
    ...
    print(f"\n3. Adding documents in batches of {batch_size} (delay: {delay}s between batches)...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Add a PDF document to the vector store with rate limiting')
